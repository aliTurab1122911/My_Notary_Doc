...
$sqll = "SELECT * FROM profile WHERE userr = '".$_SESSION['Uid']."' OR NAME = '".$_SESSION['Uid']."'";          
   $resultt = $conn->query($sqll);
   $roww=mysqli_fetch_assoc($resultt);
   
            $userrr = $roww["userr"];
            $nameee = $roww["NAME"];
            $N_name = $roww["Notary_Name"];
            $N_pub = $roww["Notary_Public"];
            $Com_s = $roww["Comm_State"];
            $Com_n = $roww["Comm_No"];
            $com_e = $roww["Comm_Expire"];
            $Stamp = $roww["Stamp"];
          

echo '          
    <div id="2reset" class="txt_box">
         <h1 id="dasheader" style="text-align:center;">MyNotaryDoc Profile</h1>
              <div class="container">
                <div class="lcu" id="lcfu">
                  <label class="tt_box">Title: '.$N_name.' </label><br>
                  <label class="tt_box">Notary Public: '.$N_pub.'</label><br>
                  <label class="tt_box">Commision State: '.$Com_s.' </label><br>
                  <label class="tt_box">Commision Number: '.$Com_n.' </label><br>
                  <label class="tt_box">Commision Expire: '.$com_e.' </label>
                </div>
                    <div class="rc" id="rcf">
                        <div class="picture">
                        <img style="max-width: 170px;max-height: 170px;" src="images/'.$Stamp.'" alt="#">
                        </div>
                    </div>
              </div>
    </div>
